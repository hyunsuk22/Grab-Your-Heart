					...
					if(Physics.Raycast(ray, out hit))
					{
						foreach(GameObject obj in draggableObjects)
						{
							if(hit.collider.gameObject == obj)
							{
                                ...
                                //오브젝트의 오디오/스테이지 데이터 가져오기.
                                if (draggableObjects[0] == obj)
                                {
                                    //기존 생성되어있던 라인제거
                                    Destroy(GameObject.FindGameObjectWithTag("line"));
                                    //Debug.Log("Grip Drag Obj[0]");
                                    _stage[0].SetActive(true);
                                    _stage[1].SetActive(false);
                                    _stage[2].SetActive(false);
                                }
                                else if (draggableObjects[1] == obj)
                                {                    
                                    Destroy(GameObject.FindGameObjectWithTag("line"));
                                    _stage[0].SetActive(false);
                                    _stage[1].SetActive(true);
                                    _stage[2].SetActive(false);
                                }
                                else if (draggableObjects[2] == obj)
                                {
                                    Destroy(GameObject.FindGameObjectWithTag("line"));
                                    _stage[0].SetActive(false);
                                    _stage[1].SetActive(false);
                                    _stage[2].SetActive(true);
                                }
